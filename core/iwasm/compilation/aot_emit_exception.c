...
aot_compile_emit_fence_nop(AOTCompContext *comp_ctx, AOTFuncContext *func_ctx)
{
    ...
    if (!(value = LLVMBuildCall2(comp_ctx->builder, func_type,
                                 func, args, 2, "call_aot_raise"))) {
    ...
    if (!(inline_asm =
                LLVMGetInlineAsm(ty, asm_string, strlen(asm_string),
                                "~{dirflag},~{fpsr},~{flags}", 27, true,
                                false, LLVMInlineAsmDialectATT, false))) {
        aot_set_last_error("llvm build nop asm failed.");
        return false;
    }
    free(asm_string);
    if (!(value = LLVMBuildCall2(comp_ctx->builder, ty, inline_asm, NULL, 0,
                                    ""))) {
        aot_set_last_error("llvm build call nop failed.");
        return false;
    }
